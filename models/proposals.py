...
class Proposal(Base):
    ...
    ngo_profile_id = Column(UUID(as_uuid=True), ForeignKey("ngo_profiles.id"), nullable=False)
    funding_opportunity_id = Column(Integer, nullable=False)  # References ReqAgent's funding_opportunities
    ...
    alignment_score = Column(Float, nullable=True)  # How well it matches funding criteria
    ...
    # Status & Workflow
    status = Column(String(50), default="draft", nullable=False)  # draft, reviewed, finalized, submitted
    version = Column(Integer, default=1, nullable=False)  # Version number
    parent_proposal_id = Column(UUID(as_uuid=True), ForeignKey("proposals.id"), nullable=True)  # For revisions
    
    # Export Information
    exported_formats = Column(JSON, nullable=True)  # List of formats exported (pdf, docx, etc.)
    ...
    funding_opportunity_snapshot = Column(JSON, nullable=True)  # Snapshot of funding opp at generation time
