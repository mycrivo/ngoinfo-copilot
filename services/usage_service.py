...
class UsageService:
    ...
    async def get_usage_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            usage_result = await self.db_session.execute(
                select(func.coalesce(func.sum(UsageLedger.count), 0))
                .where(
    ...
    async def record_usage(
        ...
        try:
            ...
            logger.info(f"Recorded usage: user={user_id}, action={action_type}, count={count}")
            return True
    ...
    async def check_rate_limit(self, user_id: str, action_type: str, limit_per_minute: int) -> bool:
